# Insect COI workflow for DPIRD researchers

#### by Jack Scanlan, 2024-11 --- current to commit [54a0191](https://github.com/AVR-biosecurity-bioinformatics/freyr/commit/54a019127fd1820e44d7ee7849dbe5dbd757e7c3)

This tutorial will help you run `freyr` for experiments utilising the 'freshwater' COI primers `fwhF2-fwhR2n` on mixed arthropod samples (focusing on insects).
...
`freyr` has four software dependencies:
- `java` version 11 or greater
- `nextflow`
- a container platform tool, preferably one of `docker`, `shifter`, `singularity` or `apptainer`
- `git` 

`nextflow` itself requires that a `java` version >=17 is installed; at AgVic we use version 17, which you can install [here](https://www.oracle.com/java/technologies/javase/jdk17-archive-downloads.html). Note that `freyr` will not work with [standalone distributions](https://git-scm.com/downloads/linux) of `nextflow`. 

`freyr` runs using software containers, which require a container platform tool to be available. We use [`shifter`](https://docs.nersc.gov/development/containers/shifter/) at AgVic, but [`docker`](https://docs.docker.com/engine/install/), [`singularity`](https://apptainer.org/admin-docs/master/installation.html) and [`apptainer`](https://apptainer.org/docs/user/main/quick_start.html#installation) are common alternative tools that should work with `freyr`. Make sure at least one of these is installed according to the tool's documentation.

[`git`](https://git-scm.com/downloads/linux) is also used to download the `freyr` pipeline files from GitHub.

### Install `nextflow` 

Installing `nextflow` is easy, if you have `java` available in your path:

```
# change to your home directory or wherever you would like to install
cd ~
...
> NOTE: Installing `nextflow` only needs to be done once before any particular user uses `freyr` for the first time -- you shouldn't need to repeat this step for subsequent runs of this pipeline, or any other `nextflow` pipeline.
...
### Demultiplex your sequencing reads
