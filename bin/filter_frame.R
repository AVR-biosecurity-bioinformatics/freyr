...
read_tracking_out <- 
    ...
    dplyr::rename("filter_frame" = frame_filter) %>%
    tidyr::pivot_longer(
        cols = !c(seq_name, sequence, filter_frame),
        names_to = "sample_id",
        values_to = "abundance"
    ) %>%
...
readr::write_csv(read_tracking_out, paste0("filter_frame_",pcr_primers,"_readsout.csv"))
